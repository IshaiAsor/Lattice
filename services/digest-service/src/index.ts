...
import { actionReadRequestedConsumer } from './consumers/action-read-requested.consumer';
import { actionDispatchHistoryConsumer } from './consumers/action-dispatch-history.consumer';
...
async function main() {
  ...
  await consume(ch, QUEUES.DEVICE_HEARTBEAT, deviceHeartbeatConsumer(ch));
  await consume(ch, QUEUES.ACTION_REQUESTED, actionRequestedConsumer(ch));
  await consume(ch, QUEUES.ACTION_READ_REQUESTED, actionReadRequestedConsumer(ch));
  await consume(ch, QUEUES.ACTION_DISPATCH_HISTORY, actionDispatchHistoryConsumer());
  ...
  log.info(
    'consumers started (telemetry, device-status, device-heartbeat, action-requested, action-read-requested, action-dispatch-history, action-result, picture-requested, ota-incoming)',
  );
