...
// The Arduino-ESP32 core bundles a precompiled esp32-camera driver (2023-era) with no
// autofocus support at all — upstream espressif/esp32-camera only added an AF API in
// v2.1.5 (2026), and that newer driver can't be linked in here because the framework
// ships its own static libesp32-camera.a that predates it (see CameraService.h). This
// talks to the OV5640's internal AF microcontroller directly over the sensor's existing
...
// OV5640 AF command/status registers (VCM microcontroller mailbox).
static const int CMD_MAIN      = 0x3022;
static const int CMD_ACK       = 0x3023;
static const int CMD_FW_STATUS = 0x3029;
