...
#ifdef MOCK_CAMERA
        _cameraReady = true;
        Serial.println("[Camera] MOCK_CAMERA: skipping hardware init (LiveStreamHttp)");
        return;
#endif
...
#ifdef MOCK_CAMERA
        httpFrameService.sendFrame(MOCK_FRAME_BYTES, MOCK_FRAME_LEN, false);
        Serial.printf("[Camera] Mock frame sent (%u bytes) via HTTP stream\n", (unsigned)MOCK_FRAME_LEN);
        return "";
#else
